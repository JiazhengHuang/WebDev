...
public function verifyCredentials($accountName, $psw){
    $stmt = $this->DB->prepare("SELECT username, password FROM users where username = :accountName");
    $stmt->bindParam( ':accountName', $accountName );
    $stmt->execute();
    $array = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    //$hashPW = password_hash($psw, PASSWORD_DEFAULT);
...
// Complete these four straightfoward functions and run as a CLI application
    ...
    public function check_if_username_exist($accountname){
...
// // $theDBA->addQuote('This page shares my best articles to read on topics like health, happiness, creativity, productivity and more. The central question that drives my work is, “How can we live better?” To answer that question, I like to write about science-based ways to solve practical problems.', 'A');
...
// // $theDBA->decreaseRating(1);
// // $theDBA->increaseRating(1);
// // $theDBA->increaseRating(1);
// // $arr = $theDBA->getAllQuotations();
// // echo $arr[0]['rating'];
